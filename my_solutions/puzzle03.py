...
def outputNext3Cards(code, ind):
    ...
    if code == 1:
        second, third, fourth = small, medium, large
    elif code == 2:
        second, third, fourth = small, large, medium
    elif code == 3:
        second, third, fourth = medium, small, large
    elif code == 4:
        second, third, fourth = medium, large, small
    elif code == 5:
        second, third, fourth = large, small, medium
    else:
        second, third, fourth = large, medium, small
...
def ComputerAssistant():
    ...
    while len(cards) < 5:
        ...
        if deck[n] not in cards:
            ...
            numsuits[n // 13] += 1
            if numsuits[n // 13] > 1:
                pairsuits.append(n // 13)
        i += 1

    pairsuit = choosePairSuit(pairsuits, cardsuits, cnumbers)
...
def choosePairSuit(pairsuits, cardsuits, cnumbers):
    if len(pairsuits) == 1:
        return pairsuits[0]
    suit1, suit2 = pairsuits[0], pairsuits[1]
    numbersInSuit1, numbersInSuit2 = [], []
    for i in range(5):
        if cardsuits[i] == suit1:
            numbersInSuit1.append(cnumbers[i])
        elif cardsuits[i] == suit2:
            numbersInSuit2.append(cnumbers[i])
    minDistanceForSuit1 = getMinDistanceForSuit(numbersInSuit1)
    minDistanceForSuit2 = getMinDistanceForSuit(numbersInSuit2)
    if minDistanceForSuit1 < minDistanceForSuit2:
        return suit1
    else:
        return suit2
...
def getMinDistanceForSuit(numbersInSuit):
    ...
    for pair in combinations(numbersInSuit, 2):
        ...
        if distance <= 0 or distance > 6:
            distance = (pair[1] - pair[0]) % 13
        if distance < minDistanceForSuit:
            minDistanceForSuit = distance
    return minDistanceForSuit
